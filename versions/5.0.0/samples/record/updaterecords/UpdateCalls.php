...
class UpdateCalls
{
    ...
    public static function updatecalls($moduleAPIName)
    {
        $recordOperations = new RecordOperations($moduleAPIName);
        $bodyWrapper = new BodyWrapper();
        $records = array();
        $record1 = new Record();
        $record1->setId("3477061112241");
        // Schedule a call
        $whoId = new Record();
        $whoId->setId("3477061185851");
        $record1->addFieldValue(Calls::WhoId(), $whoId);
        $whatId = new Record();
        $whatId->setId("3477061189101");
        $record1->addFieldValue(Calls::WhatId(), $whatId);
        $record1->addFieldValue(Calls::CallType(), new Choice("Outbound"));
        // $record1->addFieldValue(Calls::OutgoingCallStatus(), new Choice("Scheduled"));
        $record1->addFieldValue(Calls::CallStartTime(), date_create("2023-07-30T11:03:06+05:30")->setTimezone(new \DateTimeZone(date_default_timezone_get())));
        $user = new Users();
        $user->setId("344042540332");
        $record1->addKeyValue("Call_Owner", $user);
        $record1->addFieldValue(Calls::Subject(), "call Scheduled");
        $record1->addFieldValue(Calls::Reminder(), new Choice("5 minutes before"));
        $record1->addFieldValue(Calls::CallPurpose(), new Choice("Demo"));
        $record1->addFieldValue(Calls::CallAgenda(), "agenda to make call");
        $record1->addKeyValue('$se_module', "Accounts");

        // log a call
        // $whoId1 = new Record();
        // $whoId1->setId("3432434234434456");
        // $record1->addFieldValue(Calls::WhoId(), $whoId1);
        // $whatId1 = new Record();
        // $whatId1->setId("3205456023230");
        // $record1->addFieldValue(Calls::WhatId(), $whatId1);
        // $record1->addFieldValue(Calls::CallType(), new Choice("Outbound"));
        // $record1->addFieldValue(Calls::Subject(), "call");
        // $record1->addFieldValue(Calls::CallDuration(), "10:00");
        // $record1->addFieldValue(Calls::OutgoingCallStatus(), new Choice("Completed"));
        // $record1->addKeyValue("Voice_Recording__s", "recordingofcall.com");
        // $record1->addFieldValue(Calls::CallStartTime(), date_create("2023-06-02T11:03:06+05:30")->setTimezone(new \DateTimeZone(date_default_timezone_get())));
        // $record1->addFieldValue(Calls::CallPurpose(), new Choice("Demo"));
        // $record1->addFieldValue(Calls::CallAgenda(), "agenda to make call");
        // $record1->addFieldValue(Calls::CallResult(), "Not Interested");
        // $record1->addFieldValue(Calls::Description(), "description");
        // $record1->addKeyValue('$se_module', "Accounts");
...
$moduleAPIName = "Calls";
UpdateCalls::initialize();
UpdateCalls::updateCalls($moduleAPIName);
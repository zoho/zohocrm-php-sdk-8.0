...
class CreateContacts
{
    ...
    public static function createContacts(string $moduleAPIName)
    {
        ...
        $record1->addFieldValue(Contacts::Email(), 'abc12@zoho.com');
        $record1->addFieldValue(Contacts::Title(), 'Title');
        $record1->addFieldValue(Contacts::Phone(), '1000101');
        $record1->addFieldValue(Contacts::Department(), 'Department name');
        $record1->addFieldValue(Contacts::OtherPhone(), '123123');
        $record1->addFieldValue(Contacts::HomePhone(), '100010001');
        $record1->addFieldValue(Contacts::Mobile(), '9820');
        $record1->addFieldValue(Contacts::Fax(), 'fax');
        $record1->addFieldValue(Contacts::Assistant(), 'Assistant');
        $record1->addFieldValue(Contacts::DateOfBirth(), new \DateTime('1990-12-01'));
        ...
        $account_name->setId("440248001179025");
        $record1->addFieldValue(Contacts::AccountName(), $account_name);
        $vendor_name = new Record();
        $vendor_name->setId("440248001054948");
        $record1->addFieldValue(Contacts::VendorName(), $vendor_name);
        $reporting_to = new Record();
        $reporting_to->setId("3004300030405022");
        $record1->addFieldValue(Contacts::ReportingTo(), $reporting_to);
        // Address info
        $record1->addFieldValue(Contacts::MailingCity(), 'Mailing City');
        $record1->addFieldValue(Contacts::MailingCountry(), 'Mailing Country');
        $record1->addFieldValue(Contacts::MailingState(), 'Mailing State');
        $record1->addFieldValue(Contacts::MailingStreet(), 'Mailing Street');
        $record1->addFieldValue(Contacts::MailingZip(), 'Mailing Zip');
        $record1->addFieldValue(Contacts::OtherCity(), 'Other city');
        $record1->addFieldValue(Contacts::OtherCountry(), 'other country');
        $record1->addFieldValue(Contacts::OtherState(), 'other state');
        $record1->addFieldValue(Contacts::OtherStreet(), 'other street');
        $record1->addFieldValue(Contacts::OtherZip(), 'other zip');
        //
        $record1->addFieldValue(Contacts::Description(), 'description');
        ...
        $record1->addKeyValue("Date_1", new \DateTime('2023-12-12'));
...
CreateContacts::initialize();
CreateContacts::createContacts($moduleAPIName);
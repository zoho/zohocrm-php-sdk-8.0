...
class UnshareBulkEmails
{
    ...
    public static function unshareBulkEmails($moduleAPIName)
    {
        $recordShareEmailOperations = new RecordShareEmailOperations($moduleAPIName);
        $request = new BodyWrapper();
        $request->setIds(["34771023", "347720"]);
        $response = $recordShareEmailOperations->unshareBulkEmails($request);
        if ($response != null) {
...
UnshareBulkEmails::initialize();
UnshareBulkEmails::unshareBulkEmails("Leads");
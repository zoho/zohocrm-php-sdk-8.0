...
class UpdateDeals
{
    ...
    public static function updateDeals(string $moduleAPIName)
    {
        $recordOperations = new RecordOperations($moduleAPIName);
        $bodyWrapper = new BodyWrapper();
        $records = array();
        $record1 = new Record();
        $record1->setId("3234234");
        $record1->addFieldValue(Deals::DealName(), 'New Deal');
        $record1->addFieldValue(Deals::Amount(), 100.0);
        $record1->addFieldValue(Deals::ClosingDate(), new \DateTime("2023-12-10"));
        $record1->addFieldValue(Deals::Stage(), new Choice("Qualification"));
        $record1->addFieldValue(Deals::Type(), new Choice("New Business"));
        $record1->addFieldValue(Deals::Probability(), 10);
        $record1->addFieldValue(Deals::NextStep(), 'Next Step');
        $record1->addFieldValue(Deals::ExpectedRevenue(), 120.0);
        ...
        $account_name->setId("30323213");
        $record1->addFieldValue(Deals::AccountName(), $account_name);
        $contact_name = new Record();
        $contact_name->setId("30323130213");
        $record1->addFieldValue(Deals::AccountName(), $contact_name);
        ...
        $linkingRecord = new Record();
        $linkingRecord->setId("4402488");
        $record1->addKeyValue("Lookup_4", $linkingRecord);
...
$moduleAPIName = "Deals";
UpdateDeals::initialize();
UpdateDeals::updateDeals($moduleAPIName);
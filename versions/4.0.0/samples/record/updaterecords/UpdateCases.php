...
class UpdateCases
{
    ...
    public static function updateCases($moduleAPIName)
    {
        $recordOperations = new RecordOperations($moduleAPIName);
        $bodyWrapper = new BodyWrapper();
        $records = array();
        $record1 = new Record();
        $record1->setId("3477061072611");
        $record1->addFieldValue(Cases::Subject(), "New Case");
        $record1->addFieldValue(Cases::Type(), new Choice("Question"));
        $productName = new Record();
        $productName->setId("347706116719010");
        $record1->addFieldValue(Cases::ProductName(), $productName);
        $record1->addFieldValue(Cases::CaseOrigin(), new Choice("Email"));
        $record1->addFieldValue(Cases::Status(), new Choice("On Hold"));
        $record1->addFieldValue(Cases::ReportedBy(), "none");
        $record1->addFieldValue(Cases::Phone(), "3021103");
        $contactName = new Record();
        $contactName->setId("3477061185851");
        $record1->addFieldValue(Cases::RelatedTo(), $contactName);
        $accountName = new Record();
        $accountName->setId("3477061189101");
        ...
        $dealName = new Record();
        $dealName->addKeyValue("id", "3477061182981");
        $record1->addFieldValue(Cases::DealName(), $dealName);
        $record1->addFieldValue(Cases::Description(), "description");
        $record1->addFieldValue(Cases::InternalComments(), "comments");
        $record1->addFieldValue(Cases::Solution(), "Solution for Problem");
        ...
        $fileDetail1->setFileIdS("ae9c7cefa4123d729ad87c6d90b0bd44183d280");
        array_push($fileDetails, $fileDetail1);
        $fileDetail2 = new FileDetails();
        $fileDetail2->setFileIdS("ae9c7cefa416d90b0bd44183d2de");
...
UpdateCases::updateCases($moduleAPIName);
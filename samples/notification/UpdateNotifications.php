...
class UpdateNotifications
{
    ...
    public static function updateNotifications()
    {
        ...
        $events = array("Accounts.all");
        $notification->setEvents($events);
        
        $expiryTime = date_create("2026-01-31T23:59:59")
            ->setTimezone(new \DateTimeZone(date_default_timezone_get()));
        $notification->setChannelExpiry($expiryTime);
        
        $notification->setToken("UPDATED_TOKEN_FOR_VERIFICATION");
        $notification->setNotifyUrl("https://www.example.com/notifications/updated");
        
        array_push($notifications, $notification);
        
        $bodyWrapper->setWatch($notifications);
        
        $response = $notificationsOperations->updateNotifications($bodyWrapper);
                            ...
                            if (is_array($value)) {
                                ...
                                foreach ($value as $item) {
                                    if ($item instanceof Notification) {
                                        ...
                                        echo("  Notify URL: " . $item->getNotifyUrl() . "\n");
                                    }
                                }
                            } else {
                                echo($key . ": " . $value . "\n");
